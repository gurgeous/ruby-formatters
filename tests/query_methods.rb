...
module ActiveRecord
  module QueryMethods
    ...
    VALID_UNSCOPING_VALUES = Set.new(%i[where select group order lock
    ...
    def uniq!(name)
      if values = @values[name] && (values.is_a?(Array) && !values.empty?)
        values.uniq!
      end
      self
    end
    ...
    def build_join_buckets
      ...
      if joins.last.is_a?(ActiveRecord::Associations::JoinDependency) && (joins.last.base_klass == klass)
    ...
    STRUCTURAL_VALUE_METHODS = (
      ...
      %i[extending where having unscope references annotate optimizer_hints]
    ).freeze # :nodoc:
